...
private:
    ...
    auto _sendRequest(Operation op, const HttpRequest& request, std::span<const std::byte> extraData = {})
        -> Task<HttpReply>;
...
inline auto HttpSession::sendRequest(Operation op, HttpRequest request, std::span<const std::byte> extraData)
    -> Task<HttpReply> {
    int n = 0;
    while (true) {
#if 1
        ...
        auto [result, timeout] =
            co_await WhenAny(_sendRequest(op, request, extraData), Sleep(request.transferTimeout()));
        if (timeout) {
        ...
        // Check redirect
        constexpr std::array redirectCodes = {301, 302, 303, 307, 308};
...
inline auto HttpSession::setCookieJar(HttpCookieJar* cookieJar) -> void { mCookieJar = cookieJar; }
inline auto HttpSession::setProxy(const Url& proxy) -> void {
    mProxy = proxy;
    // Drop all caches
    mConnections.clear();
}
inline auto HttpSession::_sendRequest(Operation op, const HttpRequest& request, std::span<const std::byte> payload)
    -> Task<HttpReply> {
    ...
    while (true) {
        ...
        if (reply) {
            reply->mTransferDuration =
                std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now() - now);
        }
        co_return reply;
    }
}
inline auto HttpSession::_readReply(Operation op, const HttpRequest& request, Connection connection)
    -> Task<HttpReply> {
    HttpReply reply;
...
inline auto HttpSession::_readHeaders(Connection& con, HttpReply& reply) -> Task<void> {
    ...
    if (std::from_chars(codeString.data(), codeString.data() + codeString.size(), reply.mStatusCode).ec !=
        std::errc()) {
        co_return Unexpected(Error::HttpBadReply);
    }
    ...
    if (!contentLength.empty()) {
        ...
        while (true) {
            ...
            if (!lenString || lenString->empty()) {
                co_return Unexpected(lenString.error());
            }
            if (lenString->empty()) {
                co_return Unexpected(Error::HttpBadReply);
            }
...
inline auto HttpSession::_connect(const Url& url) -> Task<Connection> {
    ...
    for (auto iter = mConnections.begin(); iter != mConnections.end(); ++iter) {
        if (iter->endpoint == endpoint) {
            // Cache hint
            ::fprintf(stderr, "[Http] Using cached connection on %s\n", endpoint.toString().c_str());
...
inline auto HttpSession::_connectWithProxy(const Url& url) -> Task<Connection> {
    ...
    for (auto iter = mConnections.begin(); iter != mConnections.end(); ++iter) {
        if (iter->host == host && iter->port == port) {
            // Cache hint
            ::fprintf(stderr, "[Http] Using cached connection on %s:%d, proxyed\n", iter->host.c_str(), iter->port);
            auto con = std::move(*iter);
            mConnections.erase(iter);
            co_return con;
        }
    ...
    co_return Connection{.client       = std::move(client),
...
inline auto HttpReply::text() -> Task<std::string> { co_return mContent; }
inline auto HttpReply::content() -> Task<std::vector<uint8_t> > {
    std::vector<uint8_t> vec(mContent.begin(), mContent.end());
    co_return vec;
}
inline auto HttpReply::statusCode() const -> int { return mStatusCode; }
inline auto HttpReply::status() const -> std::string_view { return mStatus; }
inline auto HttpReply::headers() const -> const HttpHeaders& { return mResponseHeaders; }
inline auto HttpReply::transferDuration() const -> std::chrono::milliseconds { return mTransferDuration; }
