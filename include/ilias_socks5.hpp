...
inline Socks5Client::Socks5Client(IoContext& ctxt, const IPEndpoint& serverEndpoint)
    : mServer(serverEndpoint), mClient(ctxt, mServer.family()) {}
inline Socks5Client::~Socks5Client() {}

inline auto Socks5Client::setServer(const IPEndpoint& endpoint) -> void { mServer = endpoint; }
...
inline auto Socks5Client::send(const void* buf, size_t bufSize) -> Task<size_t> { return mClient.send(buf, bufSize); }
inline auto Socks5Client::recv(void* buf, size_t bufSize) -> Task<size_t> { return mClient.recv(buf, bufSize); }
inline auto Socks5Client::shutdown() -> Task<void> { return mClient.shutdown(); }
